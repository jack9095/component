...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        calcIndicatorRect();
        if (mIndicatorStyle == STYLE_TRIANGLE) {  // 三角形指示器
            if (mIndicatorHeight > 0) {
                mTrianglePaint.setColor(mIndicatorColor);
                mTrianglePath.reset();  // 清除之前绘制的path
                // 倒三角形
                mTrianglePath.moveTo(paddingLeft + mIndicatorRect.left, height - mIndicatorHeight);  // 起始点
                mTrianglePath.lineTo(paddingLeft + mIndicatorRect.left / 2 + mIndicatorRect.right / 2, height); // 终止点
                mTrianglePath.lineTo(paddingLeft + mIndicatorRect.right, height - mIndicatorHeight); // 终止点
...
//                canvas.rotate(180);
        ...
        } else if (mIndicatorStyle == STYLE_BLOCK) {  // 背景色块指示器
            if (mIndicatorHeight < 0) {
